...
(function (global) {
  ...
  function start(es, url, options) {
    ...
    var xhr = isFetchSupported && TransportOption === undefined ? undefined : new XHRWrapper(
        TransportOption !== undefined ?
          new TransportOption() :
          getBestXHRTransport()
      );

    var transport = TransportOption != null && typeof TransportOption !== "string" ? new TransportOption() : (
          xhr == undefined ?
            new FetchTransport() :
            new XHRTransport()
        );
    ...
    var onStart = function (status, statusText, contentType, method, headers) {
      if (currentState === CONNECTING) {
        if (status === 200 && contentType !== undefined) { // FIXME: && contentTypeRegExp.test(contentType)
          currentState = OPEN;
          wasActivity = true;
          retry = initialRetry;
          es.readyState = OPEN;
          ...
          if (status !== 200) {
            ...
            message = `HTTP STATUS: ${status} - TEXT ${statusText}`;
          } else {
